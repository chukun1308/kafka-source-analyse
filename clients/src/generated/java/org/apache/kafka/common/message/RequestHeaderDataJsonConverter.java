...
public class RequestHeaderDataJsonConverter {
    public static RequestHeaderData read(JsonNode _node, short _version) {
        ...
        JsonNode _initialPrincipalNameNode = _node.get("initialPrincipalName");
        if (_initialPrincipalNameNode == null) {
            _object.initialPrincipalName = null;
        } else {
            if (_initialPrincipalNameNode.isNull()) {
                _object.initialPrincipalName = null;
            } else {
                if (!_initialPrincipalNameNode.isTextual()) {
                    throw new RuntimeException("RequestHeaderData expected a string type, but got " + _node.getNodeType());
                }
                _object.initialPrincipalName = _initialPrincipalNameNode.asText();
            }
        }
        JsonNode _initialClientIdNode = _node.get("initialClientId");
        if (_initialClientIdNode == null) {
            _object.initialClientId = null;
        } else {
            if (_initialClientIdNode.isNull()) {
                _object.initialClientId = null;
            } else {
                if (!_initialClientIdNode.isTextual()) {
                    throw new RuntimeException("RequestHeaderData expected a string type, but got " + _node.getNodeType());
                }
                _object.initialClientId = _initialClientIdNode.asText();
            }
        }
        return _object;
    }
    public static JsonNode write(RequestHeaderData _object, short _version) {
        ...
        if (_version >= 2) {
            if (_object.initialPrincipalName != null) {
                _node.set("initialPrincipalName", new TextNode(_object.initialPrincipalName));
            }
        }
        if (_version >= 2) {
            if (_object.initialClientId != null) {
                _node.set("initialClientId", new TextNode(_object.initialClientId));
            }
        }
        return _node;
    }
