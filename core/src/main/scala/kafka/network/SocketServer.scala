...
class SocketServer(val config: KafkaConfig,
  ...
  private def startDataPlaneProcessorsAndAcceptors(authorizerFutures: Map[Endpoint, CompletableFuture[Void]]): Unit = {
    // 获取broker间通讯所用的监听器，默认是 PLAINTEXT
  ...
  private def createDataPlaneAcceptorsAndProcessors(dataProcessorsPerListener: Int,
    ...
    endpoints.foreach { endpoint =>
      ...
      // 创建多个Processor线程，具体数目由num.network.threads决定
      addDataPlaneProcessors(dataPlaneAcceptor, endpoint, dataProcessorsPerListener)
      // 将<监听器，Acceptor线程>对保存起来，统一管理
...
private[kafka] class Acceptor(val endPoint: EndPoint,
  ...
  // 创建底层的NIO Selector对象
  // Selector对象负责执行底层实际的I/O操作，监听连接请求，读写请求等
  private val nioSelector = NSelector.open()
  // Broker端创建对象的ServerSocketChannel对象
  ...
  private[network] def addProcessors(newProcessors: Buffer[Processor], processorThreadPrefix: String): Unit = synchronized {
    ...
    if (processorsStarted.get) { // 若Processor线程池已经启动
      // 启动新的newProcessors线程
  ...
  def run(): Unit = {
    ...
    startupComplete()
    try {
      // 当前使用的Processor序号，从0开始，最大值 num.network.threads
...
private[kafka] class Processor(val id: Int,
  ...
  // Response的临时队列，主要执行Response的回调逻辑
    ...
    try {
      while (isRunning) {
        try {
          ...
          // 发送response，并将response放入到inflightResponses中
          processNewResponses()
          // 获取对应的SocketChannel上准备就绪的I/O操作
          poll()
          // 将接收到的request放入request队列中
          processCompletedReceives()
          // 为临时的Response队列中的Response执行回掉
      ...
      try {
        currentResponse match {
          ...
          case response: SendResponse =>
            // 发送Response，并将response放入inflightResponse
  ...
  protected[network] def sendResponse(response: RequestChannel.Response, responseSend: Send): Unit = {
    ...
    if (openOrClosingChannel(connectionId).isDefined) {
      ...
      // 将response加入到inflightResponse的队列中
  ...
  private def processCompletedSends(): Unit = {
    ...
    selector.completedSends.forEach { send =>
      try {
        // 取出对应的inflightResponses中的Response
  ...
  private def configureNewConnections(): Unit = {
    ...
    while (connectionsProcessed < connectionQueueSize && !newConnections.isEmpty) {
      ...
      try {
        ...
        //用selector注册该channel，注册OP_READ事件
