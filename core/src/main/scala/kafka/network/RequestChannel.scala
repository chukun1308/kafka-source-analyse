...
object RequestChannel extends Logging {
  ...
  class Request(val processor: Int,  // Processor线程的序号，即这个请求是由哪个Processor线程处理
                val context: RequestContext, // 标识请求上下文信息
                val startTimeNanos: Long,  // Request对象被创建的时间
                memoryPool: MemoryPool, // 内存缓冲区
                @volatile private var buffer: ByteBuffer, // 真正保存Request对象内容的字节缓冲区
...
class RequestChannel(val queueSize: Int,
  ...
  def removeProcessor(processorId: Int): Unit = {
    // 从processors的线程池里面移除给定的processor线程
  ...
  def sendResponse(response: RequestChannel.Response): Unit = {
    ...
    // 找到response对应的Processor线程，即request当初是由哪个Processor线程处理的
    val processor = processors.get(response.processor)
    // The processor may be null if it was shutdown. In this case, the connections
    // are closed, so the response is dropped.
    // 将response对象放置到对应Processor线程的队列中
    if (processor != null) {
