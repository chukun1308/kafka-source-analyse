...
abstract class AbstractFetcherThread(name: String, // 线程名称
  ...
  // topicPartition 要对哪个分区下的副本执行截断操作
  // truncationState ： offset + 截断状态
  ...
  // partitionMap 一组要读取的分区列表，分区是否可读取取决于PartitionFetchState中的状态
  ...
  private def maybeFetch(): Unit = {
    val fetchRequestOpt = inLock(partitionMapLock) {
      ...
      // 处理出错的分区，处理方式主要是将这个分区追加到有序map的结尾，等待后面重试
      handlePartitionsWithErrors(partitionsWithError, "maybeFetch")
      // 如果当前没有可以读取的分区，则等待fetchBackOffMs时间后重试
  ...
  private def maybeTruncate(): Unit = {
    ...
    // 对于有Leader Epoch值分区，将日志截断到Leader Epoch值对应的位移值处
    if (partitionsWithEpochs.nonEmpty) {
      truncateToEpochEndOffsets(partitionsWithEpochs)
    }
    // 对于没有Leader Epoch值分区，将日志截断到高水位处
      ...
      inLock(partitionMapLock) {
        responseData.forKeyValue { (topicPartition, partitionData) =>
          Option(partitionStates.stateValue(topicPartition)).foreach { currentFetchState =>
            ...
            // 处理条件：
            // 1. 要获取位移值和之前已保存的下一条待获取位移值相等
            // 2. 当前分区处于可获取的状态
            if (fetchPartitionData != null && fetchPartitionData.fetchOffset == currentFetchState.fetchOffset && currentFetchState.isReadyForFetch) {
              // 获取 response中Leader Epoch的值
              ...
              partitionData.error match {
                ...
                case Errors.OFFSET_OUT_OF_RANGE =>
                  ...
                  // 若Leader Epoch的值比Leader所在的Broker上的Epoch要新
                case Errors.UNKNOWN_LEADER_EPOCH =>
                  debug(s"Remote broker has a smaller leader epoch for partition $topicPartition than " +
                    s"this replica's current leader epoch of ${currentFetchState.currentLeaderEpoch}.")
                  partitionsWithError += topicPartition

                  // 若Leader Epoch的值比Leader所在的Broker上的Epoch要旧
                case Errors.FENCED_LEADER_EPOCH =>
...
case class PartitionFetchState(fetchOffset: Long,
  ...
  // 分区处于截断状态的条件是：副本处于Truncating状态且未被推迟执行
