...
class KafkaApis(val requestChannel: RequestChannel, // 请求通道
                ...
                val groupCoordinator: GroupCoordinator, // 消费者协调器组件
  ...
  override def handle(request: RequestChannel.Request): Unit = {
    try {
      ...
      // 根据请求头部信息中的apiKey字段，判断属于哪类请求
      // 然后调用响应的hand****方法
      // 如：新增RPC协议类型
      //    1.添加新的apikeys标识新请求类型
      //    2.增加新的case分支
      //    3.增加对应的hand****方法
  ...
  def handleListGroupsRequest(request: RequestChannel.Request): Unit = {
    ...
    if (authorize(request.context, DESCRIBE, CLUSTER, CLUSTER_NAME)) {
      ...
      // 直接使用刚才拿到的Group数据封装进Response返回
      sendResponseMaybeThrottle(request, requestThrottleMs =>
        createResponse(requestThrottleMs, groups, error))
    } else {
      // 找出Clients对哪些Group有GROUP资源的DESCRIBE的权限，返回这些group
  ...
  def handleCreateTopicsRequest(request: RequestChannel.Request): Unit = {
    ...
    if (!controller.isActive) {
      ...
      // 是否具有CLUSTER CREATE的权限，否则查看是否具有TOPIC资源的CREATE权限
  ...
  private[server] def authorize(requestContext: RequestContext,
    ...
    authorizer.forall { authZ =>
      // 获取待鉴权的资源类型，常见的资源类型 如 TOPIC，GROUP，CLUSTER等
