...
class LogSegment private[log] (val log: FileRecords,
  ...
  def append(largestOffset: Long,
    ...
    if (records.sizeInBytes > 0) {
      ...
      // 判断索引是否超过范围，标准是largestOffset与日志段起始位移的差值是否在 [0,Int.MAXVALUE]之间
      ...
      // 更新最大时间戳以及最大时间戳所属消息的位移属性
      if (largestTimestamp > maxTimestampSoFar) {
        maxTimestampSoFar = largestTimestamp
        offsetOfMaxTimestampSoFar = shallowOffsetOfMaxTimestamp
      }
      // append an entry to the index (if needed)
      // 插入索引到 index文件（稀疏索引）timeIndex文件
   ...
   * @param maxSize The maximum number of bytes to include in the message set we read  能读到的最大字节数
   ...
   * from the end of the log and index. 用于日志恢复：broker启动时从磁盘加载所有的日志段信息到内存中，接着创建LogSegment对象实例
   *
  ...
  def recover(producerStateManager: ProducerStateManager, leaderEpochCache: Option[LeaderEpochFileCache] = None): Int = {
    ...
    // 判断日志当前段的总字节数是不是比读到的字节到到，如果是，说明日志段读入了非法字节，这时需要截断处理
