...
case class LogAppendInfo(var firstOffset: Option[Long],
                         ...
                         var offsetOfMaxTimestamp: Long, // 消息集合最大的消息时间戳所属的位移值
                         var logAppendTime: Long,
                         var logStartOffset: Long,
                         var recordConversionStats: RecordConversionStats,
                         sourceCodec: CompressionCodec,
                         targetCodec: CompressionCodec,
                         shallowCount: Int, // 消息的批次数，每个消息批次下，可能包含多条消息
                         validBytes: Int, // 消息写入的总字节数
                         offsetsMonotonic: Boolean, // 消息的位移值是否递增
                         lastOffsetOfFirstBatch: Long, // 首个消息批次的最后一条消息的LEO，0.11.0.0版本之后与lastOffset相同
...
class Log(@volatile private var _dir: File,
   ...
   *
   *  follower副本从leader副本获得消息后更新高水位，一旦拿到消息，就必须更新高水位
   *
  ...
  def updateHighWatermark(hw: Long): Long = {
    // 新的高水位一定需要在logStartOffset 与 logEndOffset之间
  ...
  private def fetchHighWatermarkMetadata: LogOffsetMetadata = {
    ...
    // 保存当前的高水位的本地变量，避免多线程的修改
    val offsetMetadata = highWatermarkMetadata
    // 没有获得完成的高水位元数据
    if (offsetMetadata.messageOffsetOnly) {
      // 加锁
      lock.synchronized {
        // 通过读日志拿到所有副本的最小消息位移，以此作为高水位
  ...
  private def completeSwapOperations(swapFiles: Set[File]): Unit = {
    for (swapFile <- swapFiles) {
      ...
      // 确认之前删除日志段是否成功，是否还存在于oldSegments中
      val oldSegments = logSegments(swapSegment.baseOffset, swapSegment.readNextOffset).filter { segment =>
        segment.readNextOffset > swapSegment.baseOffset
      }
      // 如果存在，直接把.swap文件重命名为.log文件
  ...
  private def loadSegments(): Long = {
    ...
    // 删除上次Failure遗留下来的各种临时文件 （.cleaned、.swap、 .deleted 等）返回合法的.swap文件
  ...
  private def recoverLog(): Long = {
    ...
    // 恢复unflushed日志段操作做完了，如果logSegments不为空
    ...
    // 恢复unflushed日志段操作做完了，如果logSegments为空
    if (logSegments.isEmpty) {
      // no existing segments, create a new mutable segment beginning at logStartOffset
      // 至少创建一个空的日志段，以logStartOffset为日志段的起始位置，并加入日志段的集合中
      addSegment(LogSegment.open(dir = dir,
      ...
      if (appendInfo.shallowCount == 0) appendInfo
        ...
        lock synchronized {
          ...
          if (assignOffsets) {
            ...
            // 使用给定的位移值，无需分配
            if (!appendInfo.offsetsMonotonic) // 确保消息位移值单调递增性
          ...
          // 执行日志切分，当前日志容量可能无法容纳新的消息集合，因此有必要创建一个新的segment
          ...
          maybeDuplicate match {
            ...
            case None =>
              ...
              // 是否是要刷盘处理,与Broker端配置参数 flush.messages 有关,一般不配置，交给操作系统去刷盘
              // 但有时为了保证数据可靠性，可以配置此参数。
  ...
  private def analyzeAndValidateRecords(records: MemoryRecords,
    ...
    records.batches.forEach { batch =>
      ...
      // 消息格式Version 2的消息批次，起始位移值必须从 0 开始
      ...
      // lastOffset大于 batch的lastOffset，违反位移递增性
      ...
      // 检查消息的大小是否受限，是否大于Broker端配置的max.message.bytes这个值
      ...
      // 更新maxTimestamp，offsetOfMaxTimestamp
  ...
  def read(startOffset: Long,
           maxLength: Int,
           isolation: FetchIsolation, // 设置读取消息的隔离级别，多用于Kafka的事务
    ...
    maybeHandleIOException(s"Exception while reading from $topicPartition in dir ${dir.getParent}") {
      ...
      // 读取的消息的startOffset == maxOffset.messageOffset,说明是空消息
      if (startOffset == maxOffsetMetadata.messageOffset)
        ...
        while (!done) {
          ...
          // 调用LogSegment的read方法，读取消息
          fetchDataInfo = segment.read(startOffset, maxLength, maxPosition, minOneMessage)
          if (fetchDataInfo != null) {
            if (includeAbortedTxns)
              fetchDataInfo = addAbortedTransactions(startOffset, segmentEntry, fetchDataInfo)
          } else segmentEntry = segments.higherEntry(segmentEntry.getKey) // 如果没读到数据，去下一个日志段试试
  ...
  private def deleteSegments(deletable: Iterable[LogSegment], reason: SegmentDeletionReason): Int = {
    maybeHandleIOException(s"Error while deleting segments for $topicPartition in dir ${dir.getParent}") {
      ...
      if (numToDelete > 0) {
        ...
        // 不允许删除所有的日志对象，如果一定要删除，那么先创建一个新的，再把前面的全部删除
