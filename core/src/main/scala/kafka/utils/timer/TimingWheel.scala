 ...
 */
@nonthreadsafe
private[timer] class TimingWheel(tickMs: Long,  // 滴答一次的时长，类似于手表向前推进一次的时间，对于秒针来说，就是一秒
                                 wheelSize: Int, // 每层时间轮上的Bucket数量
                                 startMs: Long, // 时间轮被创建时的起始时间戳
                                 taskCounter: AtomicInteger, // 这一层时间轮的总任务数
                                 queue: DelayQueue[TimerTaskList] // 将所有Bucket按照过期时间排序的延迟队列
                                ) {

  // 这层时间轮的总时长
  private[this] val interval = tickMs * wheelSize
  // 时间轮下所有的Bucket对象，也就是TimerTaskList对象
  ...
  // 按需分配的上层时间轮，当有新的定时任务到达时，会尝试将其放入第1层时间轮。
  // 如果第一层时间轮无法容纳定时任务的超时时间，就创建并配置第二层时间轮，以此类推
  ...
  def add(timerTaskEntry: TimerTaskEntry): Boolean = {
    ...
    // 如果任务已经被取消了，则无需添加，直接返回
    if (timerTaskEntry.cancelled) {
      // Cancelled
      false
    } else if (expiration < currentTime + tickMs) { // 任务已经过期了
      // Already expired
      false
    } else if (expiration < currentTime + interval) { // 超时时间在本层时间轮覆盖时间范围内
  ...
  def advanceClock(timeMs: Long): Unit = {
    // 向前驱动到的时间点要超过Bucket的时间范围，才是有意义的推进
    if (timeMs >= currentTime + tickMs) {
      // 更新当前时间currentTime到下一个Bucket的起始时间点
