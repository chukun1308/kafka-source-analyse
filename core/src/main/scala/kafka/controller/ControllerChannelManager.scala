...
class ControllerChannelManager(controllerContext: ControllerContext,
  ...
  def addBroker(broker: Broker): Unit = {
    ...
    brokerLock synchronized {
      ...
      if (!brokerStateInfo.contains(broker.id)) {
        // 将新的Broker加入到Controller管理，并创建对应的RequestSendThread
        addNewBroker(broker)
  ...
  protected def startRequestSendThread(brokerId: Int): Unit = {
    // 获取指定brokerId的requestSendThread
...
class RequestSendThread(val controllerId: Int, // Controller所在Broker的Id
                        ...
                        val brokerNode: Node, // 目标的Broker节点
                        val config: KafkaConfig, // kafka的配置信息
                        val time: Time,
  ...
  override def doWork(): Unit = {
    ...
    try {
      ...
      if (clientResponse != null) {
        ...
        // 此时的Response请求必须是LEADER_AND_ISR、STOP_REPLICA、UPDATE_METADATA中的一种类型
