...
class ControllerDeletionClient(controller: KafkaController, zkClient: KafkaZkClient) extends DeletionClient {
  ...
  override def deleteTopic(topic: String, epochZkVersion: Int): Unit = {
    ...
    // 删除 /admin/delete_topic/<topic>节点
    zkClient.deleteTopicDeletions(Seq(topic), epochZkVersion)
  }

  // 删除 /admin/delete_topics下的给定topic字节点
 ...
 * @param controller
 */
class TopicDeletionManager(config: KafkaConfig, // kafka的配置信息，保存Broker端参数
                           ...
                           client: DeletionClient // DeletionClient接口，实现主题删除
                           ) extends Logging {
  ...
  private def completeDeleteTopic(topic: String): Unit = {
    ...
    // 注销分区变更器，防止删除过程中因分区数据变更导致监听器被触发，引起状态不一致
    client.mutePartitionModifications(topic)
    // 获取该主题下处于ReplicaDeletionSuccessful状态的所有副本
    val replicasForDeletedTopic = controllerContext.replicasInState(topic, ReplicaDeletionSuccessful)
    // controller will remove this replica from the state machine as well as its partition assignment cache
    // 利用副本状态机将这些副本对象转换为NonExistentReplica状态
  ...
  private def onTopicDeletion(topics: Set[String]): Unit = {
    ...
    // 给集群中所有的Broker发送元数据更新请求，告诉他们不需要在处理这些主题的数据
  ...
  private def resumeDeletions(): Unit = {
    ...
    topicsQueuedForDeletion.foreach { topic =>
      ...
      // 若该主题的所有副本已经是ReplicaDeletionSuccessful状态，即主题已经被删除
      if (controllerContext.areAllReplicasInState(topic, ReplicaDeletionSuccessful)) {
        // clear up all state for this topic from controller cache and zookeeper
        // 调用completeDeleteTopic方法完成后续操作即可，即清除元数据和zk相关的状态
        completeDeleteTopic(topic)
        info(s"Deletion of topic $topic successfully completed")
      }
        // 如果主题尚未开始删除并且主题当前无法执行删除
      else if (!controllerContext.isAnyReplicaInState(topic, ReplicaDeletionStarted)) {
        // if you come here, then no replica is in TopicDeletionStarted and all replicas are not in
        // TopicDeletionSuccessful. That means, that either given topic haven't initiated deletion
        // or there is at least one failed replica (which means topic deletion should be retried).
        if (controllerContext.isAnyReplicaInState(topic, ReplicaDeletionIneligible)) {
          // 把主题加入待重试主题列表中，用于后续重试
