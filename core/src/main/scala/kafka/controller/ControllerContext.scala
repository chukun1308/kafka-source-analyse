...
class ControllerContext {
  ...
  var preferredReplicaImbalanceCount = 0 // 优先副本选举数量
  ...
  private val liveBrokerEpochs = mutable.Map.empty[Int, Long] // 运行中的每个Broker的Epoch
  // Controller当前的Epoch，实际上就是zk中/controller_epoch节点的值，Controller在整个Kafka集群中的版本号
  var epoch: Int = KafkaController.InitialControllerEpoch
  // /controller_epoch节点的dataVersion值，epochZkVersion主要是防止 Zombie Controller，
  // 也就说，原先在老Controller任期内的Controller操作 再去操作新的Controller不能成功执行，
  // 因为新的Controller的epochZkVersion比老的大
  var epochZkVersion: Int = KafkaController.InitialControllerEpochZkVersion

  val allTopics = mutable.Set.empty[String] // 集群主题列表
  val partitionAssignments = mutable.Map.empty[String, mutable.Map[Int, ReplicaAssignment]] // 集群主题分区的副本分配情况
  // <主题、分区, 分区主副本、isr、并且leaderEpoch> 的相关映射信息
  ...
  val topicsToBeDeleted = mutable.Set.empty[String] // 待删除的主题列表
  ...
  private def updatePartitionStateMetrics(partition: TopicPartition,
    ...
    if (!isTopicDeletionInProgress(partition.topic)) {
      ...
      // 若当前状态不是OfflinePartition，即离线状态，并且要变为离线状态
      if (currentState != OfflinePartition && targetState == OfflinePartition) {
        offlinePartitionCount = offlinePartitionCount + 1
        // 当前已经是离线状态了，但是targetState不是离线状态
