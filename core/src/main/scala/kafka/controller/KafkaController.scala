...
class KafkaController(val config: KafkaConfig, // kafka配置信息
                      ...
                      time: Time, // 提供时间服务
                      metrics: Metrics, // 实现指标监控服务
                      initialBrokerInfo: BrokerInfo, // Broker节点信息，包括主机名，端口号，监听器
                      initialBrokerEpoch: Long, // broker Epoch的值，用于隔离老Controller发送的请求
  ...
  // controller端通道管理类，负责controller向Broker发送请求
  ...
  // controller节点zk监听器，监听/controller节点的变更
  ...
  // 主题删除zk监听器，监听/admin/delete_topics
  ...
  // 当前controller所在的broker id
  @volatile private var activeControllerId = -1
  // 离线分区的总数
  @volatile private var offlinePartitionCount = 0
  // 满足preferred leader选举条件的总数
  ...
  def startup() = {
    ...
    // 启动ControllerEventThread线程，开始处理事件队列中的ControllerEvent事件
    eventManager.start()
  }
  ...
  private def onBrokerStartup(newBrokers: Seq[Int]): Unit = {
    ...
    // 给集群中现有Broker发送元数据更新请求，广播新增的broker
    ...
    // 给新增的Broker发送元数据更新请求，令他们同步集群当前的所有分区数据
    ...
    // 将新增Broker上所有的副本设置为Online状态，即可用状态
    ...
    // 为新增的broker注册BrokerModificationsHandler监听器
  ...
  private def onBrokerFailure(deadBrokers: Seq[Int]): Unit = {
    ...
    // deadBrokers：给定一组已终止的运行的broker了列表
    // 更新Controller元数据信息，将给定Broker从元数据的replicasOnOfflineDirs移除
    deadBrokers.foreach(controllerContext.replicasOnOfflineDirs.remove)
    // 找出这些待移除的Broker上的所有副本，执行相应的操作
    val deadBrokersThatWereShuttingDown =
      deadBrokers.filter(id => controllerContext.shuttingDownBrokerIds.remove(id))
    if (deadBrokersThatWereShuttingDown.nonEmpty)
      info(s"Removed ${deadBrokersThatWereShuttingDown.mkString(",")} from list of shutting down brokers.")
    val allReplicasOnDeadBrokers = controllerContext.replicasOnBrokers(deadBrokers.toSet)
    // 将待移除的副本置为不可用状态，即（offline）
  ...
  private def onBrokerUpdate(updatedBrokerId: Int): Unit = {
    ...
    // 给集群所有的brokert发送UpdateMetadataRequest，让他们去更新元数据
    sendUpdateMetadataRequest(controllerContext.liveOrShuttingDownBrokerIds.toSeq, Set.empty)
  }
  ...
  private def processStartup(): Unit = {
    // 注册ControllerChangeHandler zk监听器
  ...
  private def maybeResign(): Unit = {
    // 判断当前的broker是不是controller
    val wasActiveBeforeChange = isActive
    // 注册controllerChangeHandler监听器
    zkClient.registerZNodeChangeHandlerAndCheckExistence(controllerChangeHandler)
    // 获取当前controller所在的broker id
    activeControllerId = zkClient.getControllerId.getOrElse(-1)
    // 如果该Broker原来是controller，现在不是了
  ...
  private def elect(): Unit = {
    ...
    try {
      // 创建/controller节点，设置zk的epoch epochZkVersion变化值
      val (epoch, epochZkVersion) = zkClient.registerControllerAndIncrementControllerEpoch(config.brokerId)
      // 设置当前的controller上下文的 epoch，epochZkVersion值
  ...
  private def processBrokerChange(): Unit = {
    // 当前broker不是controller，无权处理，返回
    if (!isActive) return
    // 从zk中获取集群broker列表
    val curBrokerAndEpochs = zkClient.getAllBrokerAndEpochsInCluster
    val curBrokerIdAndEpochs = curBrokerAndEpochs map { case (broker, epoch) => (broker.id, epoch) }
    val curBrokerIds = curBrokerIdAndEpochs.keySet
    // 获取当前controllerContext的broker列表
    val liveOrShuttingDownBrokerIds = controllerContext.liveOrShuttingDownBrokerIds
    //比较两个列表，计算新增的broker，待删除的broker
    ...
    // 为已重启的Broker执行重添加逻辑，更新 controllerContext，执行重启动broker逻辑
    ...
    // 为待移除的broker执行移除controllerContext和Broker终止逻辑
  ...
  private def processTopicChange(): Unit = {
    // 若 Controller已经关闭，直接返回
    if (!isActive) return
    // 从zk中获取当前集群所有的主题
    val topics = zkClient.getAllTopicsInCluster(true)
    // 找出新的主题，即当前元数据中不存在,zk存在的主题
    val newTopics = topics -- controllerContext.allTopics
    // 找出已删除的主题，即当前元数据存在，在zk中不存在的主题
    val deletedTopics = controllerContext.allTopics.diff(topics)
    // 更新Controller元数据
    controllerContext.setAllTopics(topics)
    // 为新增的主题和已经删除的主题执行后续的处理操作
    registerPartitionModificationsHandlers(newTopics.toSeq)
    val addedPartitionReplicaAssignment = zkClient.getFullReplicaAssignmentForTopics(newTopics)
    deletedTopics.foreach(controllerContext.removeTopic)
    // 为新增的主题更新元数据缓存中的副本分配条目
    addedPartitionReplicaAssignment.foreach {
      case (topicAndPartition, newReplicaAssignment) => controllerContext.updatePartitionFullReplicaAssignment(topicAndPartition, newReplicaAssignment)
    }
    info(s"New topics: [$newTopics], deleted topics: [$deletedTopics], new partition replica assignment " +
      s"[$addedPartitionReplicaAssignment]")
    // 调整新增主题所有分区以及所属副本的运行状态为上线状态
  ...
  private def processTopicDeletion(): Unit = {
    ...
    if (config.deleteTopicEnable) {
      if (topicsToBeDeleted.nonEmpty) {
        ...
        // 待删除主题插入到删除等待集合，交由topicDeletionManager处理
      ...
      // 清除zk下 /admin/delete_topics下的字节点
...
class TopicDeletionHandler(eventManager: ControllerEventManager) extends ZNodeChildChangeHandler {
  // zk节点 /admin/delete_topics
  override val path: String = DeleteTopicsZNode.path
  // 向事件队列中写入TopicDeletion事件
...
class ControllerChangeHandler(eventManager: ControllerEventManager) extends ZNodeChangeHandler {
  ...
  // 监听/controller删除事件,Reelect事件是重选举事件
